#!/usr/bin/perl -w

use CGI qw/:all/;
use CGI::Carp qw(fatalsToBrowser warningsToBrowser);

$max_number_to_guess = 99;

#prints response header and html header
print <<eof;
Content-type: text/html

<!DOCTYPE html>
<head>
  <title>Guess A Number</title>
  <link href="guess_number.css" rel="stylesheet">
</head>
<body>
eof

warningsToBrowser(1);
$number_to_guess = param('number_to_guess');
$guess = param('guess');
$game_over = 0;

if (defined $number_to_guess and defined $guess) {
	#sanitises parameters
	$guess =~ s/\D//g;
	$number_to_guess =~ s/\D//g;

	#evaluates the provided guess
	if ($guess == $number_to_guess) {
		print "You guessed right, it was $number_to_guess.\n";
		$game_over = 1;
	} elsif ($guess < $number_to_guess) {
		print "Its higher than $guess.\n";
	} else {
		print "Its lower than $guess.\n";
	}

} else {
    $number_to_guess = 1 + int(rand $max_number_to_guess);
    print "I've thought of number 0..$max_number_to_guess\n";
}

if ($game_over) {
	#prints end of game page
	print <<eof;
<form method="POST" action="">
  <input type="submit" value="Play Again" class="guess_button">
</form>
eof
} else {
	#prints guessing form
	print <<eof;
<form method="POST" action="">
  <input type="textfield" name="guess" class="guess_textfield">
  <input type="hidden" name="number_to_guess" value="$number_to_guess">
</form>
eof
}

#prints end of html
print <<eof;
</body>
</html>
eof
